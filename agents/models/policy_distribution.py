...
class PolicyDistributionModelOLD:
...
class PolicyDistributionModel:
    ...
    def __str__(self):
        return '\t'.join('TeammateDistributionModel({teammate}) Prob={prob}'.format(teammate=index, prob=prob)
                         for index, prob in self.policy_distribution.items())
    ...
    def __hash__(self):
        if not self.__hash:
            self.__hash = hash(tuple(self.policy_distribution))
