    ...
    def func(belief_state, action):
        ...
        for state, state_prob in belief_state.items():
            for resulting_state, resulting_state_prob in transition(state, action).items():
                if resulting_state not in resulting_state_dist:
                    resulting_state_dist[resulting_state] = 0
...
def po_utility(utility):
    def func(old_belief_state, action, new_belief_state):
        return new_belief_state.expectation({new_state:
                                                 sum(utility(old_state, action, new_state) * prob for old_state, prob in
                                                     old_belief_state.items())
